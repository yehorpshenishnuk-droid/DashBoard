...
def get_power_status():
    """Парсинг актуального графика с API Yasno v2"""
    ...
    try:
        print(f"DEBUG: Fetching power for group {group_name}...")
        ...
        schedules = schedule_component.get('schedule', {})
        region_data = schedules.get('dtek-krem', {}) # Область
        
        if not region_data:
            region_data = schedules.get('dtek-kem', {}) # Город

        # Вот эта строка покажет нам, какие группы доступны (только в логах Render)
        print(f"DEBUG: Available groups: {list(region_data.keys())}")

        group_info = region_data.get(group_name)
        
        if not group_info and "." in group_name:
            fallback = group_name.split(".")[0] + ".1"
            group_info = region_data.get(fallback)

        if group_info:
            return {
                "has_power": group_info.get("has_power"),
                "schedule_text": group_info.get("schedule_text", ""),
                "group": group_name
            }
            
    except Exception as e:
        print(f"DEBUG Power Parse Error: {e}")
